    ...
    try {
        ...
        // Sửa: bỏ check car.status !== 'available'
        // Chỉ cần xe tồn tại và không phải bảo trì
        if (!car || car.status === 'maintenance') return res.redirect('/cars');
        ...
        // Double-check conflict theo ngày
        const conflict = await Booking.findOne({
            car:    carId,
            status: { $in: ['pending', 'confirmed', 'active'] },
            startDate: { $lt: end },
        ...
        const totalDays   = Math.max(1, Math.ceil((end - start) / (1000 * 60 * 60 * 24)));
        const totalAmount = car.pricePerDay * totalDays;  // tính đúng tổng tiền
        ...
        res.render('booking/confirm', {
            car, startDate, endDate, totalDays,
            user: req.user,
    ...
    try {
        ...
        const [totalCars, rentedCars, recentCars, newCustomers, revenueResult, allTimeResult] = await Promise.all([
            Car.countDocuments(),
            ...
            // Doanh thu tháng này (đơn hoàn thành trong tháng)
            Booking.aggregate([
                {
                    $match: {
                        status:    'completed',
        ...
        res.render('admin/dashboard', {
            ...
            stats: {
                ...
                monthBookings:   revenueResult[0]?.count  || 0,
                allTimeRevenue:  allTimeResult[0]?.total  || 0,
                allTimeBookings: allTimeResult[0]?.count  || 0
            },
            recentCars,
            recentBookings
        });
